<template>
    <Modal :show="show" @close="emit('close')">
        <div
            class="p-8 bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
        >
            <div
                class="flex items-center justify-between mb-8 pb-4 border-b border-gray-800"
            >
                <h3
            ...
            <section>
                ...
                <div
                    ...
                    class="py-8 text-center bg-gray-800/30 rounded-xl border border-dashed border-gray-700"
                >
                ...
                <ul v-else class="space-y-2">
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <div
                                class="h-8 w-8 rounded-full bg-red-900/30 flex items-center justify-center text-red-400 text-xs font-bold border border-red-500/10"
                            >
        ...
        <AreYouSureModal
            ...
            :text="$t('confirm_delete_kapsule')"
            :actionText="$t('delete')"
            :isRedButton="true"
            @close="showAreYouSureDeleteModal = false"
            @accepted="deleteKapsule()"
        />
    </Modal>
</template>
...
const showAreYouSureDeleteModal = ref(false);
...
const promptDelete = () => {
    showAreYouSureDeleteModal.value = true;
};

const deleteKapsule = () => {
    router.delete(route("kapsules.destroy", { kapsule: props.kapsule.id }), {
        onSuccess: (page) => {
